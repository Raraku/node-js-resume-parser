...
class ResumeParserCli(object):
    def __init__(self):
        ...
        self.__parser.add_argument(
            "-r", "--remotefile", help="remote path for resume file to be extracted"
        )
        self.__parser.add_argument(
            "-re", "--custom-regex", help="custom regex for parsing mobile numbers"
        )
        self.__parser.add_argument(
            "-sf",
            "--skillsfile",
        ...
        self.__parser.add_argument(
            "-e", "--export-format", help="the information export format (json)"
        )
        self.__parser.add_argument(
            "-o", "--export-filepath", help="the export file path"
        )
    ...
    def export_data(self, exported_data, args):
        ...
        if args.export_format:
            ...
            return json.dumps(exported_data, indent=4, sort_keys=True)
    ...
    def extract_resume_data(self):
        ...
        if args.remotefile:
            return self.export_data(
                self.__extract_from_remote_file(
                    args.remotefile, args.skillsfile, args.custom_regex
                ),
                args,
            )
        ...
        if args.file and not args.directory:
            return self.export_data(
                self.__extract_from_file(args.file, args.skillsfile, args.custom_regex),
                args,
            )
        elif args.directory and not args.file:
            return self.export_data(
                self.__extract_from_directory(
                    args.directory, args.skillsfile, args.custom_regex
                ),
                args,
            )
        else:
