...
async function parse(PreparedFile, cbReturnResume) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    // 2 parse profiles
    row = rows[i] = parseDictionaryProfiles(row, Resume, rows, i);
    // 3 parse titles
      ...
      return new Promise(function (resolve) {
        ...
        var checkTimer = setInterval(function () {
          ...
          if (profilesWatcher.inProgress === 0 || i > 5) {
            ...
            // console.log(cbReturnResume(Resume));
            parsedResume = cbReturnResume(Resume);
 ...
 * @returns {*}
 */
function parseDictionaryProfiles(row, Resume, rows, i) {
